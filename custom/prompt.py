...
你是一名中医专家，请按照如下案例的方式进行抽取[核心临床信息]。要求[核心临床信息]只列出以”;“分隔的核心信息，不含任何换行符等无关字符。
--------------
{context_str}
[临床资料]: {clinical_info}
[核心临床信息]: 
...
你的推理过程应该是：1.根据[核心临床信息]进行[病机推断]；2.根据[病机推断]筛选出[核心病机]；3.筛选出最有可能正确的三个[核心病机]；4.根据[核心病机]在[病机选项]中选出对应的[病机答案]。
注意：正确选项最多选三个。要求：答案只包含正确选项，不含任何误导性。
---------------------
{context_str}
---------------------
[临床资料]: {clinical_info}
...
你的推理过程应该是：1.根据[核心病机]进行[证候推断]；2.根据[证候推断]筛选出最有可能正确的两个[核心证候]；3.根据[核心证候]在[证候选项]中选出对应的[证候答案]。
注意：正确选项最多选两个。要求：答案只包含正确选项，不含任何误导性。
---------------------
{context_str}
---------------------
[核心临床信息]: {core_clinical_info}
