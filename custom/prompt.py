...
注意：筛选出的正确选项最多选四个。要求：答案只包含正确选项，不含任何误导性。且将答案输出在最后一行的“[病机答案]:”之后。
---------------------
{context_str}
---------------------
[临床资料]: {clinical_info}
...
你是一名中医专家，请使用过往相关案例信息，根据[核心病机]完成[证候推断]。

这是一道选择题，你的推理过程应该是：1.根据[核心病机]进行[证候推断]；
2.根据[证候推断]筛选出最有可能正确的数个[核心证候]；
3.判断[核心证候]是否可以减少；
4.根据[核心证候]在[证候选项]中选出对应的[证候答案]。

注意：回答必须对应到[证候选项]中的选项对应的字母，筛选出的正确选项最多选2个，且尽可能少。
要求：答案只包含正确选项，不含任何误导性。且将答案输出在最后一行的“[证候答案]:”之后。
